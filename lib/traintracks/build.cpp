  ...
  if (K == 1)
    {
      ...
      std::cerr << " in traintrack::traintrack::traintrack(N,K).\n";
      std::exit(1);
    }
...
traintrack::traintrack(const int N, const int K1, const int K2)
{
  if (K1 == 1 || K2 == 1)
    {
      std::cerr << "Specify monogons via N, the number of punctures,";
      std::cerr << " in traintrack::traintrack::traintrack(N,K1,K2).\n";
      std::exit(1);
    }
...
traintrack::traintrack(const int N, const int K1, const int K2, const int K3)
{
  if (K1 == 1 || K2 == 1 || K3 == 1)
    {
      std::cerr << "Specify monogons via N, the number of punctures,";
      std::cerr << " in traintrack::traintrack::traintrack(N,K1,K2,K3).\n";
      std::exit(1);
    }
						...
						const intVec& Kv)
{
  ...
  for (int i = 0; i < (int)Kv.size(); ++i)
	...
	{
	  std::cerr << "Specify monogons via N, the number of punctures,";
	  std::cerr << " in traintrack::traintrack::traintrack(N,Kv)\n";
	  std::exit(1);
	}
    }
  ...
  if (boundary_prongs(N,Kv) == 1 && Kv.size() > 2)
    {
      ...
      std::cerr << " in traintrack::traintrack::ttbuild_monogoncusps.\n";
      std::exit(1);
    }
      ...
      for (int p = 0; p < Multigon(m).prongs(); ++p)
	{
	  for (int e = 0; e < Multigon(m).edges(p); ++e)
	      ...
	      if (ua0 && ua1)
		{
		  ...
		  std::cerr << " in traintrack::traintrack::capoff.\n";
		  std::exit(1);
		}
	      if (ua0 || ua1)
		{
  ...
  if (Multigon(L).prongs() != 1)
    {
      ...
      std::cerr << " in traintrack::traintrack::monogon_to_multigon\n";
      std::exit(1);
    }
