...
- Added one-step sign/orientation checks that non-permutation folds inject
  exactly one fixed-orientation infinitesimal generator into main-edge images.
- Adopted convention B (fixed global infinitesimal-loop orientation) and
  updated locked examples/assertions accordingly.
- Added internal fold-map consistency guard in `fold_traintrack_map` that
  verifies map-derived and matrix-derived main-edge transitions agree.
