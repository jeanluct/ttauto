...
Indices are the C++ ones: vertices, branches, multigons, prongs and edges
are numbered from~$0$.  (The interactive \code{ttauto} program
and the published tables print vertices from~$1$.)  Letters of a
train-track map follow the convention of the map module:
the $n$ main edges are the letters $1,\dots,n$, the sides of the multigons
are $n+1,\dots,n+n_{\mathrm{s}}$ where $n_{\mathrm{s}}$ is the total number
of prongs, and a negative letter is the same edge with the opposite
orientation ($\anti e=-e$ in the code).  Identifiers of the library are
set in \code{typewriter}; file names appear only in
Appendix~\ref{sec:appendix}.  The mathematics is that of Bestvina and
Handel~\cite{Bestvina1995}, in the notation of Hall~\cite{HallBHchapter};
the train-track formalism is that of the \ttauto\
paper~\cite{Lanneau2011}.
...
its single side is a loop around the puncture.  Every edge has two ends
attached at prongs, so the structure is a graph whose vertices are
blown up into polygons (Figure~\ref{fig:blowup}).  The \emph{singularity
data} of a track is the list of its multigons with their prong numbers
and puncturedness; \ttauto\ writes it as, for instance, $1.\,1.\,1.\,1.\,(2)$
for four punctured monogons and an unpunctured 2-gon on the outer
boundary.  Tracks with the same singularity data form a \emph{stratum},
and the constructor \code{build_traintrack_list(n)} returns one
representative track per stratum of the disc with $n$ punctures.
...
four punctured monogons and an unpunctured 2-gon on the outer boundary.
