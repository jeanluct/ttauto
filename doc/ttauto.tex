...
once instead of repeatedly without lowering the dilatation.  The prune
is therefore valid only when the search is for a minimum: the repeated
path is generally pseudo-Anosov in its own right, and discarding it
loses that class.  It is off by default for that reason.  When a path
...
\code{badword_length} enables bad-word pruning, in either mode and only
when hunting a minimum; \code{min_dilatation}
